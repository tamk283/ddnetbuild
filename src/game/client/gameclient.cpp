...
int CGameClient::DDNetVersion() const { return g_Config.m_KxSpoofClientStr ? g_Config.m_KxVersionSpoofId : DDNET_VERSION_NUMBER; }
const char *CGameClient::DDNetVersionStr() const
{
        // Rebuild on every read so spoof settings changed at runtime (Settings UI)
        // take effect on the next connect, not only after a full client restart.
        UpdateDDNetVersionStr();
        return m_aDDNetVersionStr;
}
int CGameClient::ClientVersion7() const { return CLIENT_VERSION7; }

void CGameClient::UpdateDDNetVersionStr() const
{
        // Client identity spoof: when enabled, replace GAME_NAME / GAME_RELEASE_VERSION
        // / GIT_SHORTREV_HASH with user-configured values. This only affects the
        // string sent to the server in NETMSG_CLIENTVER (DDNetVersionStr); the
        // real GAME_NAME is still used for window title, HTTP UA, etc.
        if(g_Config.m_KxSpoofClientStr)
        {
                ...
                if(g_Config.m_KxSpoofGitHashEnabled)
                {
                        const char *pHash = g_Config.m_KxSpoofGitHash[0] ? g_Config.m_KxSpoofGitHash : "0";
                        str_format(m_aDDNetVersionStr, sizeof(m_aDDNetVersionStr), "%s %s (%s)", pName, pVer, pHash);
                }
                else if(GIT_SHORTREV_HASH)
                {
                        str_format(m_aDDNetVersionStr, sizeof(m_aDDNetVersionStr), "%s %s (%s)", pName, pVer, GIT_SHORTREV_HASH);
                }
                else
                {
                        str_format(m_aDDNetVersionStr, sizeof(m_aDDNetVersionStr), "%s %s", pName, pVer);
                }
        }
        else if(GIT_SHORTREV_HASH)
        {
...
void CGameClient::OnConsoleInit()
{
        ...
        m_vpAll.insert(m_vpAll.end(), {&m_Skins,
                                              ...
                                              &m_Ghost,
                                              &m_Players,
                                              &m_BotNet, // moved here so world-space renders (Pathfinder, Show Path) draw ON TOP of the map
                                              &m_Tas, // TAS fake world (fake tee + fake projectiles) render on top of map
                                              &m_AimBot, // AimBot/TriggerBot rendering (FOV/Radius circles)
                                              &m_AvoidFreeze, // Avoid Freeze - override input to avoid freeze
                                              &m_Esp, // ESP lines (Layer 0 = behind fg blocks; Layer>0 deferred to m_KinetixLines)
                                              &m_Trajectory, // v1.56.178: per-type trajectory prediction (replaces old RenderTrajectory in players.cpp)
                                              &m_MapLayersForeground,
                                              &m_KinetixLines, // deferred line overlay — renders AFTER fg map tiles (on top of blocks)
        ...
        m_vpInput.insert(m_vpInput.end(), {&m_KeyBinder, // this will take over all input when we want to bind a key
                                                  &m_Binds.m_SpecialBinds,
                                                  &m_ClickGui, // consume clicks/keys when ClickGUI is open (must be before menus/spectator so ESC closes it, not the game menu)
                                                  &m_GameConsole,
...
void CGameClient::OnInit()
{
        ...
        // Build initial m_aDDNetVersionStr from current config. The string is
        // rebuilt on every read of DDNetVersionStr() so spoof settings changed
        // at runtime (in Settings UI) take effect on the next connect.
        UpdateDDNetVersionStr();
...
void CGameClient::OnDummySwap()
{
        ...
        {
                // DDNet: PlayerOrDummy = (reset==2) ? ClDummy : (!ClDummy)
                // Multi-dummy: when reset==1, reset the PREVIOUS active slot
                // (the slot that just became non-active = m_LastActiveDummy).
                int PlayerOrDummy = (g_Config.m_ClDummyResetOnSwitch == 2) ? g_Config.m_ClDummy : m_LastActiveDummy;
                if(PlayerOrDummy >= 0 && PlayerOrDummy < MAX_DUMMIES)
                {
        ...
        // ── DDNet OnDummySwap logic, adapted for multi-dummy ───────────────────
        // Original DDNet (NUM_DUMMIES=2, shared m_DummyInput):
        ...
        //
        // In DDNet, m_DummyInput holds the CURRENT dummy's input (the slot that IS
        // the dummy before the swap). After swap, that slot becomes the new active.
        // The fire transfer gives the new active its own fire counter back.
        //
        // In Kinetix (MAX_DUMMIES=8, per-slot m_aDummyInput[D]):
        // The new active (g_Config.m_ClDummy) was previously a dummy, so its fire
        // counter is in m_aDummyInput[g_Config.m_ClDummy]. Save it BEFORE overwriting.
        const int NewActivePrevFire = m_aDummyInput[g_Config.m_ClDummy].m_Fire;

        // For each non-active D, copy m_aInputData[D] → m_aDummyInput[D].
        // This gives the OLD active (now becoming a dummy) its active input as
        // dummy input. Other inactive dummies get a refresh too (harmless — they
        // were inactive, their m_aInputData[D] was last set by copy_moves or
        // dummy_control, both of which keep m_aInputData[D] in sync with what
        // the server expects).
        for(int D = 0; D < MAX_DUMMIES; D++)
        {
                ...
                if(D != 0 && !Client()->DummyConnected(D))
                        continue;
                m_aDummyInput[D] = m_Controls.m_aInputData[D];
        }

        // Give the new active its fire counter back. This is the ONLY change to
        // m_aInputData[g_Config.m_ClDummy] — we do NOT do a full copy from
        // m_aDummyInput (that was a Kinetix addition that clobbered the player's
        // weapon state with hammer input's m_WantedWeapon=HAMMER+1).
        // Direction/jump/hook will be overwritten by the player's key state on
        // the next SnapInput() call anyway.
        m_Controls.m_aInputData[g_Config.m_ClDummy].m_Fire = NewActivePrevFire;

        m_LastActiveDummy = g_Config.m_ClDummy;
        m_IsDummySwapping = 1;
}
...
int CGameClient::OnSnapInput(int *pData, int Conn, bool Force)
{
        if(Conn == g_Config.m_ClDummy)
        {
                ...
                // SnapInput anyway so m_aInputData (read by CTas::LiveInput)
                // follows the real keys, then zero the packed input so the
                // server and the local prediction (Client()->GetInput) see a
                // standing character.
                if(m_Tas.IsJoined())
                {
        ...
        {
                if(m_aDummyFire[Conn] != 0)
                {
                        // Hammer just turned off — send a release (even fire counter).
                        m_aDummyInput[Conn].m_Fire = (m_aHammerInput[Conn].m_Fire + 1) & ~1;
                        m_aDummyFire[Conn] = 0;
                        // GUARANTEE the release is sent this frame. Without this, the
                        // `!Force && !dir && !jump && !hook` skip-check below would drop
                        // the release when the dummy is idle, leaving the server thinking
                        // the dummy is still firing → extra hammer hit on next press.
                        // This matches DDNet's implicit guarantee (DDNet's ProcessDummy-
                        // free pipeline keeps dir/jump/hook non-zero from copy_moves or
                        // the last OnDummySwap, so the skip-check rarely triggers).
                        Force = true;
                }

                if(!Force && (!m_aDummyInput[Conn].m_Direction && !m_aDummyInput[Conn].m_Jump && !m_aDummyInput[Conn].m_Hook))
                        return 0;

                mem_copy(pData, &m_aDummyInput[Conn], sizeof(m_aDummyInput[Conn]));
                return sizeof(m_aDummyInput[Conn]);
        }
        else
        {
                // Determine if auto-fly is active: dummy auto-hammers every tick when
                // the active player is within hammer radius.
                const bool autoFly = g_Config.m_KxDummyHammerAuto != 0;
                bool canHammerNow = true;
                if(autoFly)
                {
                        const int activeId = m_aLocalIds[g_Config.m_ClDummy];
                        if(activeId < 0)
                        {
                                canHammerNow = false;
                        }
                        ...
                        {
                                // Use m_Predicted.m_Pos (server-accurate) instead of m_RenderPos
                                // (interpolated, lags 2-5 ticks). m_RenderPos caused spurious
                                // hammer fires when the dummy appeared in-range visually but had
                                // already moved out of range on the server. DDNet uses
                                // m_Predicted.m_Pos for the same reason.
                                const vec2 activePos = m_aClients[activeId].m_Predicted.m_Pos;
                                const vec2 dummyPos = m_aClients[m_aLocalIds[Conn]].m_Predicted.m_Pos;

                                // v1.56.189: Configurable max distance (replaces hardcoded 50).
                                // Real DDNet hammer max reach center-to-center = ProjStartPos offset
                                // (PhysicalSize*0.75 = 21) + FindEntities radius (PhysicalSize*0.5 = 14)
                                // + target ProximityRadius (28) = 63. Slider max = 63 (v1.56.190).
                                const float maxDist = (float)g_Config.m_KxDummyHammerAutoMaxDist;

                                // v1.56.189 (fixed v1.56.191): Predict — simulate next tick.
                                // m_Vel is in units/tick, so nextPos = curPos + m_Vel.
                                // BOTH active and dummy positions are predicted (v1.56.189 only
                                // predicted active, leaving dummy at current pos — wrong, since
                                // both move every tick). Now we check the distance between the
                                // two predicted positions, which is the actual next-tick distance.
                                vec2 activeTestPos = activePos;
                                vec2 dummyTestPos = dummyPos;
                                if(g_Config.m_KxDummyHammerAutoPredict)
                                {
                                        activeTestPos = activePos + m_aClients[activeId].m_Predicted.m_Vel;
                                        dummyTestPos = dummyPos + m_aClients[m_aLocalIds[Conn]].m_Predicted.m_Vel;
                                }
                        ...
                        // Auto fly overrides custom latency — hammers every tick when in range.
                }
                else if(g_Config.m_KxDummyHammerCustomLatency)
                {
                        const int latencyTicks = std::max(1, (g_Config.m_KxDummyHammerLatencyMs + 19) / 20);
                        if(m_aDummyFire[Conn] % latencyTicks != 0)
                                canHammerNow = false;
                }
                else
                {
                ...
                // v1.56.188: Auto Fly — when dummy is out of hammer range, behave
                // as if cl_dummy_hammer is off: send normal dummy input so the user
                // can control the dummy (movement/jump/hook/weapon). Hammer input
                // (m_aHammerInput) and fire counter (m_aDummyFire) are NOT touched —
                // they stay ready for seamless resume when the dummy re-enters range.
                // Without this, dummy was stuck on hammer weapon with no user control
                // whenever autoFly was on, even when nowhere near the active player.
                if(autoFly && canHammerNow)
                {
                        const int FireDelayTicks = m_aTuning[Conn].GetWeaponFireDelay(WEAPON_HAMMER) * Client()->GameTickSpeed();
                        if(Client()->GameTick(g_Config.m_ClDummy) - m_aDummyLastFireTick[Conn] < FireDelayTicks)
                                canHammerNow = false;
                }
                ...
                // Without this, when KxDummyHammerKeepInputs is on, the server holds
                ...
                // (default) or the full custom-latency interval. Fire parity ONLY
                ...
                // Preserve inputs: when on, keep dummy's movement inputs (direction/jump/hook)
                // from m_aDummyInput EVERY tick. When off, hammer input fully overrides.
                if(g_Config.m_KxDummyHammerKeepInputs)
                {
                        m_aHammerInput[Conn].m_Direction = m_aDummyInput[Conn].m_Direction;
                        m_aHammerInput[Conn].m_Jump = m_aDummyInput[Conn].m_Jump;
                        m_aHammerInput[Conn].m_Hook = m_aDummyInput[Conn].m_Hook;
                }

                if(canHammerNow)
                {
                        // Fire this tick — flip parity to odd (press).
                        // Update aim toward active player ONLY on fire ticks (matches
                        // DDNet — aim is recomputed only when actually hammering, not
                        // every tick, so dummy doesn't jitter toward player between hits).
                        m_aDummyFire[Conn]++;
                        m_aDummyLastFireTick[Conn] = Client()->GameTick(g_Config.m_ClDummy);
                        m_aHammerInput[Conn].m_Fire = (m_aHammerInput[Conn].m_Fire + 1) | 1;
                        const vec2 Dir = m_LocalCharacterPos - m_aClients[m_aLocalIds[Conn]].m_Predicted.m_Pos;
                        m_aHammerInput[Conn].m_TargetX = (int)Dir.x;
                        m_aHammerInput[Conn].m_TargetY = (int)Dir.y;
                }
                else
                {
                        // Not firing this tick — advance counter but DON'T flip fire
                        // parity. Server sees same parity → no new trigger, but
                        // direction/jump/hook fields (updated above) still take effect.
                        // Aim (m_TargetX/Y) is NOT updated — stays at last fire value.
                        m_aDummyFire[Conn]++;
                }

                mem_copy(pData, &m_aHammerInput[Conn], sizeof(m_aHammerInput[Conn]));
                return sizeof(m_aHammerInput[Conn]);
        }
}

void CGameClient::ApplyCopyMovesTransforms(int D, const CNetObj_PlayerInput &SrcInput)
{
        // Only applies when copy moves is on; transforms the dummy's input in-place.
        if(!g_Config.m_ClDummyCopyMoves)
                return;
        if(D < 0 || D >= MAX_DUMMIES || D == g_Config.m_ClDummy)
                return;
        if(D != 0 && !Client()->DummyConnected(D))
                return;

        // --- Mirror direction (XOR) ---
        if(g_Config.m_KxDummyCopyMirrorDir)
        {
                // m_Direction: -1, 0, +1. Mirror flips the sign.
                m_aDummyInput[D].m_Direction = -m_aDummyInput[D].m_Direction;
        }

        // --- Aim transforms (only when Smart Aim or Mirror Aim enabled) ---
        const bool wantSmart = (g_Config.m_KxDummyCopyAimMode == 0) && g_Config.m_KxDummyCopySmartAim;
        const bool wantMirror = (g_Config.m_KxDummyCopyAimMode == 1) && (g_Config.m_KxDummyCopyMirrorAimX || g_Config.m_KxDummyCopyMirrorAimY);
        if(!wantSmart && !wantMirror)
                return;

        const int activeId = m_aLocalIds[g_Config.m_ClDummy];
        if(activeId < 0 || m_aLocalIds[D] < 0)
                return;
        const vec2 activePos = m_aClients[activeId].m_RenderPos;
        const vec2 dummyPos = m_aClients[m_aLocalIds[D]].m_RenderPos;

        if(wantSmart)
        {
                // Smart aim: dummy aims at the same WORLD point as the player.
                // Player's world aim = activePos + (m_TargetX, m_TargetY) from SrcInput.
                // v1.56.186: SrcInput is the (possibly delayed) source — keeps smart
                // aim in sync with movement/fire replay delay.
                const vec2 playerWorldAim = activePos + vec2(
                        (float)SrcInput.m_TargetX,
                        (float)SrcInput.m_TargetY);
                const vec2 dummyAim = playerWorldAim - dummyPos;
                m_aDummyInput[D].m_TargetX = (int)dummyAim.x;
                m_aDummyInput[D].m_TargetY = (int)dummyAim.y;
        }
        else // wantMirror
        {
                // Mirror aim: reflect the player's LOCAL aim vector by X and/or Y.
                // v1.56.186: SrcInput is the (possibly delayed) source.
                int tx = SrcInput.m_TargetX;
                int ty = SrcInput.m_TargetY;
                if(g_Config.m_KxDummyCopyMirrorAimX)
                        tx = -tx;
                if(g_Config.m_KxDummyCopyMirrorAimY)
                        ty = -ty;
                m_aDummyInput[D].m_TargetX = tx;
                m_aDummyInput[D].m_TargetY = ty;
        }
}

// v1.56.186: Copy Moves Latency — record the active player's finalized input
// each SnapInput call. Entries are timestamped by game tick so dummies can
// later replay the input from N ticks ago.
//
// We keep up to 2200 entries (= 44 seconds at 50 tps). With max latency 5000ms
// * 7 dummies = 35000ms = 1750 ticks needed, 2200 gives a safety margin. Old
// entries past the back are pruned. The vector is treated as a ring (oldest
// first) — we search back-to-front because recent entries are at the end.
void CGameClient::RecordActiveInput(const CNetObj_PlayerInput &Input)
{
        const int CurTick = Client()->GameTick(g_Config.m_ClDummy);
        SInputHistoryEntry Entry;
        Entry.m_Tick = CurTick;
        Entry.m_Input = Input;
        m_ActiveInputHistory.push_back(Entry);

        // Prune. Keep at most 2200 entries (~44s @ 50tps). Max needed depth:
        // KxCopyMovesLatency(5000ms) * MAX_DUMMIES(7) / tick_time(20ms) = 1750 ticks.
        constexpr size_t MAX_HISTORY = 2200;
        if(m_ActiveInputHistory.size() > MAX_HISTORY)
        {
                const size_t Excess = m_ActiveInputHistory.size() - MAX_HISTORY;
                m_ActiveInputHistory.erase(m_ActiveInputHistory.begin(), m_ActiveInputHistory.begin() + Excess);
        }
}

// Returns the most recent entry with tick <= (CurTick - DelayTicks), i.e. the
// input that was active `DelayTicks` ticks ago. If no entry is old enough,
// returns false (caller should fall back to live input — e.g. right after
// connect when the history is still filling).
bool CGameClient::GetDelayedActiveInput(int DelayTicks, CNetObj_PlayerInput &Out) const
{
        if(DelayTicks <= 0 || m_ActiveInputHistory.empty())
                return false;

        const int CurTick = Client()->GameTick(g_Config.m_ClDummy);
        const int TargetTick = CurTick - DelayTicks;

        // Search back-to-front — recent entries are at the end.
        for(auto it = m_ActiveInputHistory.rbegin(); it != m_ActiveInputHistory.rend(); ++it)
        {
                if(it->m_Tick <= TargetTick)
                {
                        Out = it->m_Input;
                        return true;
                }
...
void CGameClient::OnReset()
{
        ...
        // v1.56.186: clear Copy Moves Latency input history on reset (map change /
        // reconnect resets the tick counter, so old entries would be invalid).
        m_ActiveInputHistory.clear();
        ...
        for(auto &DummyInput : m_aDummyInput)
                DummyInput = {};
        for(auto &HammerInput : m_aHammerInput)
                HammerInput = {};
        for(auto &DummyFire : m_aDummyFire)
                DummyFire = 0;
...
void CGameClient::UpdatePositions()
{
	...
	// spectator position
	vec2 PfSpecCam;
	const bool PfSpecFollowCam = m_BotNet.PfSpectateUpdateCamera(PfSpecCam);
	if(m_Snap.m_SpecInfo.m_Active)
	{
		...
		else if(PfSpecFollowCam)
		{
			m_Snap.m_SpecInfo.m_Position = PfSpecCam;
...
void CGameClient::OnRender()
{
        ...
        if(m_aLocalIds[0] >= 0 && Client()->State() == IClient::STATE_ONLINE && !m_Menus.IsActive() && WasNewTick)
        {
                ...
                // Check info resend for all dummies
                for(int D = 1; D < MAX_DUMMIES; D++)
                {
                        if(m_aLocalIds[D] < 0 || !Client()->DummyConnected(D))
                                continue;
                        ...
                        if(m_aCheckInfo[D] == 0)
                        {
                                ...
                                {
                                        if(
                                                str_comp(m_aClients[m_aLocalIds[D]].m_aName, Client()->DummyName()) ||
                                                str_comp(m_aClients[m_aLocalIds[D]].m_aClan, g_Config.m_ClDummyClan) ||
                                                m_aClients[m_aLocalIds[D]].m_Country != g_Config.m_ClDummyCountry ||
                                                str_comp(m_aClients[m_aLocalIds[D]].m_aSkinName, g_Config.m_ClDummySkin) ||
                                                m_aClients[m_aLocalIds[D]].m_UseCustomColor != g_Config.m_ClDummyUseCustomColor ||
                                                m_aClients[m_aLocalIds[D]].m_ColorBody != (int)g_Config.m_ClDummyColorBody ||
                                                m_aClients[m_aLocalIds[D]].m_ColorFeet != (int)g_Config.m_ClDummyColorFeet)
                                                SendDummyInfo(false, D);
                                        else
                                                m_aCheckInfo[D] = -1;
                                }
                        }
                        ...
                        if(m_aCheckInfo[D] > 0)
                        {
                                m_aCheckInfo[D] -= minimum(Client()->GameTick(D) - Client()->PrevGameTick(D), m_aCheckInfo[D]);
                        }
                }
        }
...
void CGameClient::OnDummyDisconnect(int Dummy)
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
                return;
        m_aLocalIds[Dummy] = -1;
        m_aDDRaceMsgSent[Dummy] = false;
        m_aShowOthers[Dummy] = SHOW_OTHERS_NOT_SET;
        m_aEnableSpectatorCount[Dummy] = -1;
        m_aLastNewPredictedTick[Dummy] = -1;
        // Reset dummy input state
        mem_zero(&m_aDummyInput[Dummy], sizeof(m_aDummyInput[Dummy]));
        mem_zero(&m_aHammerInput[Dummy], sizeof(m_aHammerInput[Dummy]));
        m_aDummyFire[Dummy] = 0;
}
...
{
        if(!g_Config.m_ClPredictDummy || m_Snap.m_LocalClientId < 0)
                return false;
        // Check if at least one non-active connection has a valid local ID and isn't paused.
        // This includes D=0 (main player) when the user is controlling a dummy,
        // and D=1+ when they are connected dummies.
        for(int D = 0; D < MAX_DUMMIES; D++)
        {
                if(D == g_Config.m_ClDummy)
                        continue;
                if(D != 0 && !Client()->DummyConnected(D))
                        continue;
                if(m_aLocalIds[D] >= 0 && !m_aClients[m_aLocalIds[D]].m_Paused)
                        return true;
        }
        return false;
}
...
void CGameClient::OnMessage(int MsgId, CUnpacker *pUnpacker, int Conn, bool Dummy)
{
        ...
        if(Dummy)
        {
                if(MsgId == NETMSGTYPE_SV_CHAT && m_aLocalIds[0] >= 0)
                {
                        ...
                        // Check if any connected dummy is on a different team than the main player
                        bool DifferentTeam = false;
                        for(int D = 1; D < MAX_DUMMIES; D++)
                        {
                                if(m_aLocalIds[D] >= 0 && Client()->DummyConnected(D) &&
                                        (m_aClients[m_aLocalIds[0]].m_Team != m_aClients[m_aLocalIds[D]].m_Team ||
                                         m_Teams.Team(m_aLocalIds[0]) != m_Teams.Team(m_aLocalIds[D])))
                                {
                                        DifferentTeam = true;
                                        break;
                                }
                        }
                        if((pMsg->m_Team == 1 && DifferentTeam) || pMsg->m_Team > 1)
                        {
...
void CGameClient::OnNewSnapshot(bool DummySwapped)
{
        ...
        for(int i = 0; i < MAX_DUMMIES; i++)
        {
                ...
                // Skip dummy connections that aren't connected
                if(i >= IClient::CONN_DUMMY_START && i < MAX_DUMMIES && !Client()->DummyConnected(i))
                {
                        continue;
                }
        ...
        for(int D = 1; D < MAX_DUMMIES; D++)
        {
                if(Client()->DummyConnected(D) && (m_aEnableSpectatorCount[D] == -1 || m_aEnableSpectatorCount[D] != g_Config.m_ClShowhudSpectatorCount))
                {
                        CNetMsg_Cl_EnableSpectatorCount Msg;
                        Msg.m_Enable = g_Config.m_ClShowhudSpectatorCount;
                        Client()->SendPackMsg(D, &Msg, MSGFLAG_VITAL);
                        m_aEnableSpectatorCount[D] = g_Config.m_ClShowhudSpectatorCount;
                }
        }
        ...
        if(Client()->AnyDummyConnected() && !m_LastDummyConnected)
        {
                for(int D = 1; D < MAX_DUMMIES; D++)
                {
                        if(!Client()->DummyConnected(D))
                                continue;
                        int Conn = IClient::CONN_DUMMY_START + D - 1;
                        {
        ...
        if(ShowDistanceZoom != m_LastShowDistanceZoom || Graphics()->ScreenAspect() != m_LastScreenAspect)
        {
                ...
                if(Client()->AnyDummyConnected() && m_LastDummyConnected)
                {
                        for(int D = 1; D < MAX_DUMMIES; D++)
                        {
                                if(!Client()->DummyConnected(D)) continue;
                                Client()->SendMsg(IClient::CONN_DUMMY_START + D - 1, &Packer, MSGFLAG_VITAL);
                        }
                }
        }
        ...
        m_LastDummyConnected = Client()->AnyDummyConnected();
...
void CGameClient::ApplyPreInputs(int Tick, bool Direct, CGameWorld &GameWorld)
{
        ...
        for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
        {
                ...
                {
                        {
                                bool IsLocalId = (ClientId == m_aLocalIds[g_Config.m_ClDummy]);
                                if(!IsLocalId)
                                {
                                        for(int D = 0; D < MAX_DUMMIES; D++)
                                        {
...
void CGameClient::OnPredict()
{
        ...
        // TODO(spect-predict): anti-ping prediction does not work while spectating
        // (/pause with free view, /spec). Server side:
        //   - /spec (PAUSE_SPEC) calls CCharacter::Pause(true) which REMOVES our
        //     character from the world (character.cpp:2426-2427), so it is not
        //     sent in snapshots anymore -> m_aCharacters[LocalClientId].m_Active
        //     becomes false -> early return below kills ALL prediction.
        //   - /pause (PAUSE_PAUSED) keeps the character, but if the whole world
        //     is paused the server sets GAMESTATEFLAG_PAUSED and the block at
        //     3003 disables prediction the same way.
        // Tried: keep a CCharacter copy (m_pKxSavedChar) and re-insert it into
        // m_PredictedWorld after CopyWorld. BROKE prediction everywhere — the
        // copy ctor copies the private m_pNextTypeEntity/m_pPrevTypeEntity list
        // pointers and ~CEntity() calls GameWorld()->RemoveEntity(this), which
        // then writes through stale pointers (m_pParent->m_DestroyTick in
        // gameworld.cpp RemoveEntity) and corrupts the PredictedWorld list.
        // Needs a solution that does NOT touch OnPredict / CEntity lifetime.
        ...
        // init
        // When swapping dummies, the active connection index is temporarily
        // the new one, but the input buffer still has data from the old one.
        // XOR with m_IsDummySwapping (like original DDNet) to get the correct
        // input buffer index for the active player during the swap tick.
        int ActiveConn = g_Config.m_ClDummy ^ m_IsDummySwapping;
        ...
        // Collect all dummy characters for prediction
        // When on a non-zero dummy, also include D=0 (main player) as a predicted dummy
        CCharacter *apDummyChars[MAX_DUMMIES] = {};
        if(PredictDummy())
        {
                for(int D = 0; D < MAX_DUMMIES; D++)
                {
                        ...
                        if(D == 0)
                        {
                                // Main player is always "connected" but only predicted as dummy when not active
                                if(m_aLocalIds[0] >= 0)
                                        apDummyChars[0] = m_PredictedWorld.GetCharacterById(m_aLocalIds[0]);
                        }
                        else if(m_aLocalIds[D] >= 0 && Client()->DummyConnected(D))
                                apDummyChars[D] = m_PredictedWorld.GetCharacterById(m_aLocalIds[D]);
                }
        }
        ...
        for(int Tick = Client()->GameTick(g_Config.m_ClDummy) + 1; Tick <= Client()->PredGameTick(g_Config.m_ClDummy); Tick++)
        {
                ...
                if(Tick == Client()->PredGameTick(g_Config.m_ClDummy))
                {
                        ...
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                if(D == g_Config.m_ClDummy)
                                        continue;
                                if(apDummyChars[D])
                                        m_aClients[m_aLocalIds[D]].m_PrevPredicted = apDummyChars[D]->GetCore();
                        }
                }
                ...
                CNetObj_PlayerInput *pInputData = (CNetObj_PlayerInput *)Client()->GetInput(Tick, ActiveConn);

                // Collect dummy input data
                // Use GetInput from the input buffer only. Do NOT fall back to
                // m_aDummyInput[D] — that struct may have stale counter values
                // (m_NextWeapon/m_PrevWeapon/m_Fire) which would cause CountInput
                // to compute huge deltas and trigger spurious weapon switches.
                // When no input exists for a tick, skip it (matching original DDNet).
                CNetObj_PlayerInput *apDummyInputData[MAX_DUMMIES] = {};
                for(int D = 0; D < MAX_DUMMIES; D++)
                {
                        if(D == g_Config.m_ClDummy)
                                continue;
                        if(apDummyChars[D])
                        {
                                apDummyInputData[D] = (CNetObj_PlayerInput *)Client()->GetInput(Tick, D);
                        }
                }

                // Sort all characters by CID for correct input processing order
                // Build a list of (CID, char*, input*) tuples, sort by CID
                struct CInputEntry
                {
                        int m_Cid;
                        CCharacter *m_pChar;
                        CNetObj_PlayerInput *m_pInput;
                };
                CInputEntry aEntries[1 + MAX_DUMMIES];
                int NumEntries = 0;
                if(pInputData)
                {
                        aEntries[NumEntries].m_Cid = pLocalChar->GetCid();
                        aEntries[NumEntries].m_pChar = pLocalChar;
                        aEntries[NumEntries].m_pInput = pInputData;
                        NumEntries++;
                }
                for(int D = 0; D < MAX_DUMMIES; D++)
                {
                        if(D == g_Config.m_ClDummy)
                                continue;
                        if(apDummyChars[D] && apDummyInputData[D])
                        {
                                aEntries[NumEntries].m_Cid = apDummyChars[D]->GetCid();
                                aEntries[NumEntries].m_pChar = apDummyChars[D];
                                aEntries[NumEntries].m_pInput = apDummyInputData[D];
                                NumEntries++;
                        }
                }
                // Simple insertion sort by CID
                for(int i = 1; i < NumEntries; i++)
                {
                        CInputEntry Tmp = aEntries[i];
                        int j = i - 1;
                        while(j >= 0 && aEntries[j].m_Cid > Tmp.m_Cid)
                        {
                ...
                // Apply direct input in CID order
                for(int i = 0; i < NumEntries; i++)
                        aEntries[i].m_pChar->OnDirectInput(aEntries[i].m_pInput);

                ApplyPreInputs(Tick, true, m_PredictedWorld);

                m_PredictedWorld.m_GameTick = Tick;
                // Apply predicted input in CID order
                for(int i = 0; i < NumEntries; i++)
                        aEntries[i].m_pChar->OnPredictedInput(aEntries[i].m_pInput);
                ...
                if(Tick == Client()->PredGameTick(g_Config.m_ClDummy))
                {
                        ...
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                if(D == g_Config.m_ClDummy)
                                        continue;
                                if(apDummyChars[D])
                                        m_aClients[m_aLocalIds[D]].m_Predicted = apDummyChars[D]->GetCore();
                        }
                }
                ...
                if(Tick > m_aLastNewPredictedTick[ActiveConn])
                {
                        m_aLastNewPredictedTick[ActiveConn] = Tick;
                ...
                // check if we want to trigger predicted airjump for dummies
                if(AntiPingPlayers())
                {
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                if(D == g_Config.m_ClDummy)
                                        continue;
                                if(apDummyChars[D] && Tick > m_aLastNewPredictedTick[D])
                                {
                                        m_aLastNewPredictedTick[D] = Tick;
                                        vec2 Pos = apDummyChars[D]->Core()->m_Pos;
                                        int Events = apDummyChars[D]->Core()->m_TriggeredEvents;
                                        if(g_Config.m_ClPredict && !m_SuppressEvents)
                                                if(Events & COREEVENT_AIR_JUMP)
                                                        m_Effects.AirJump(Pos, 1.0f, 1.0f);
                                }
...
void CGameClient::CClientData::UpdateSkinInfo()
{
        ...
        const auto &&ApplySkinProperties = [&]() {
                ...
                {
                        for(int Dummy = 0; Dummy < MAX_DUMMIES; Dummy++)
                        {
...
void CGameClient::SendStartInfo7(int Dummy)
{
        ...
        {
                Msg.m_apSkinPartNames[p] = CSkins7::ms_apSkinVariables[Dummy][p];
                Msg.m_aUseCustomColors[p] = *CSkins7::ms_apUCCVariables[Dummy][p];
                Msg.m_aSkinPartColors[p] = *CSkins7::ms_apColorVariables[Dummy][p];
        }
        CMsgPacker Packer(&Msg, false, true);
        if(Msg.Pack(&Packer))
                return;
        int Conn = Dummy == 0 ? 0 : (IClient::CONN_DUMMY_START + Dummy - 1);
        Client()->SendMsg(Conn, &Packer, MSGFLAG_VITAL | MSGFLAG_FLUSH);
        m_aCheckInfo[Dummy] = -1;
}

void CGameClient::SendSkinChange7(int Dummy)
{
        ...
        int Conn = Dummy == 0 ? 0 : (IClient::CONN_DUMMY_START + Dummy - 1);
        Client()->SendMsg(Conn, &Packer, MSGFLAG_VITAL | MSGFLAG_FLUSH);
        m_aCheckInfo[Dummy] = Client()->GameTickSpeed();
}

bool CGameClient::GotWantedSkin7(int Dummy)
{
        ...
        {
                str_copy(aSkinParts[SkinPart], CSkins7::ms_apSkinVariables[Dummy][SkinPart], protocol7::MAX_SKIN_ARRAY_SIZE);
                apSkinPartsPtr[SkinPart] = aSkinParts[SkinPart];
                aUCCVars[SkinPart] = *CSkins7::ms_apUCCVariables[Dummy][SkinPart];
                aColorVars[SkinPart] = *CSkins7::ms_apColorVariables[Dummy][SkinPart];
        }
        m_Skins7.ValidateSkinParts(apSkinPartsPtr, aUCCVars, aColorVars, m_pClient->m_TranslationContext.m_GameFlags);

        for(int SkinPart = 0; SkinPart < protocol7::NUM_SKINPARTS; SkinPart++)
        {
                if(str_comp(m_aClients[m_aLocalIds[Dummy]].m_aSixup[Dummy].m_aaSkinPartNames[SkinPart], apSkinPartsPtr[SkinPart]))
                        return false;
                if(m_aClients[m_aLocalIds[Dummy]].m_aSixup[Dummy].m_aUseCustomColors[SkinPart] != aUCCVars[SkinPart])
                        return false;
                if(m_aClients[m_aLocalIds[Dummy]].m_aSixup[Dummy].m_aSkinPartColors[SkinPart] != aColorVars[SkinPart])
                        return false;
        }
...
void CGameClient::SendDummyInfo(bool Start, int Dummy)
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
                return;
        ...
        {
                if(Start)
                        SendStartInfo7(Dummy);
                else
                        SendSkinChange7(Dummy);
                return;
        }
        if(Start)
        {
...
void CGameClient::SendKill() const
{
        ...
        if(g_Config.m_ClDummyCopyMoves)
        {
                ...
                // Send kill to all connected dummies
                for(int D = 1; D < MAX_DUMMIES; D++)
                {
                        if(Client()->DummyConnected(D))
                        {
                                int Conn = IClient::CONN_DUMMY_START + D - 1;
                                Client()->SendMsg(Conn, &MsgP, MSGFLAG_VITAL);
                        }
                }
        }
}
...
void CGameClient::ConchainSpecialDummy(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback, void *pCallbackUserData)
{
        ...
        if(pResult->NumArguments())
        {
                if(g_Config.m_ClDummy != 0 && !((CGameClient *)pUserData)->Client()->DummyConnected(g_Config.m_ClDummy))
                        g_Config.m_ClDummy = 0;
        }
}
...
void CGameClient::UpdateLocalTuning()
{
        ...
        if(m_GameWorld.m_WorldConfig.m_UseTuneZones)
        {
                ...
                if(m_ActiveRecordings.any() && m_IsDummySwapping && m_aLocalTuneZone[g_Config.m_ClDummy] != m_aLocalTuneZone[Client()->LastDummy()])
                {
                        CMsgPacker Msg(NETMSGTYPE_SV_TUNEPARAMS);
                        int *pParams = (int *)&m_aTuning[g_Config.m_ClDummy];
                        for(unsigned i = 0; i < sizeof(m_aTuning[0]) / sizeof(int); i++)
...
void CGameClient::UpdatePrediction()
{
        ...
        // Collect all connected dummy characters for prediction
        // Include D=0 (main player) when it's not the active connection
        CCharacter *apDummyChars[MAX_DUMMIES] = {};
        for(int D = 0; D < MAX_DUMMIES; D++)
        {
                if(D == g_Config.m_ClDummy)
                ...
                {
                        if(m_aLocalIds[0] >= 0)
                                apDummyChars[0] = m_GameWorld.GetCharacterById(m_aLocalIds[0]);
                }
                else if(Client()->DummyConnected(D) && m_aLocalIds[D] >= 0)
                        apDummyChars[D] = m_GameWorld.GetCharacterById(m_aLocalIds[D]);
        }
        ...
        if(pLocalChar && absolute(m_GameWorld.GameTick() - Client()->GameTick(g_Config.m_ClDummy)) < Client()->GameTickSpeed())
        {
                ...
                {
                        CNetObj_PlayerInput *pInput = (CNetObj_PlayerInput *)Client()->GetInput(Tick, g_Config.m_ClDummy);
                        ...
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                if(D == g_Config.m_ClDummy)
                                        continue;
                                if(apDummyChars[D])
                                        apDummyInputs[D] = (CNetObj_PlayerInput *)Client()->GetInput(Tick, D);
                        }

                        // Sort all characters by CID for correct input processing order
                        struct CSnapInputEntry
                        {
                                int m_Cid;
                                CCharacter *m_pChar;
                                CNetObj_PlayerInput *m_pInput;
                        };
                        CSnapInputEntry aEntries[1 + MAX_DUMMIES];
                        int NumEntries = 0;
                        if(pInput)
                        {
                                aEntries[NumEntries].m_Cid = pLocalChar->GetCid();
                                aEntries[NumEntries].m_pChar = pLocalChar;
                                aEntries[NumEntries].m_pInput = pInput;
                                NumEntries++;
                        }
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                if(D == g_Config.m_ClDummy)
                                        continue;
                                if(apDummyChars[D] && apDummyInputs[D])
                                {
                                        aEntries[NumEntries].m_Cid = apDummyChars[D]->GetCid();
                                        aEntries[NumEntries].m_pChar = apDummyChars[D];
                                        aEntries[NumEntries].m_pInput = apDummyInputs[D];
                                        NumEntries++;
                                }
                        }
                        // Simple insertion sort by CID
                        for(int i = 1; i < NumEntries; i++)
                        {
                                CSnapInputEntry Tmp = aEntries[i];
                                int j = i - 1;
                                while(j >= 0 && aEntries[j].m_Cid > Tmp.m_Cid)
                                {
                                        aEntries[j + 1] = aEntries[j];
        ...
        else
        {
                ...
                if(pLocalChar)
                        if(CNetObj_PlayerInput *pInput = (CNetObj_PlayerInput *)Client()->GetInput(Client()->GameTick(g_Config.m_ClDummy), g_Config.m_ClDummy))
                ...
                for(int D = 0; D < MAX_DUMMIES; D++)
                {
                        if(D == g_Config.m_ClDummy)
                                continue;
                        if(apDummyChars[D])
                                if(CNetObj_PlayerInput *pInput = (CNetObj_PlayerInput *)Client()->GetInput(Client()->GameTick(g_Config.m_ClDummy), D))
                                        apDummyChars[D]->SetInput(pInput);
                }
        }
                ...
                if(m_Snap.m_aCharacters[i].m_Active)
                {
                        ...
                        if(!IsLocal && PredictDummy())
                        {
                                for(int D = 0; D < MAX_DUMMIES; D++)
                                {
                                        ...
                                        else if(Client()->DummyConnected(D) && i == m_aLocalIds[D])
                                        {
                                                IsLocal = true;
                                                break;
                                        }
...
void CGameClient::UpdateRenderedCharacters()
{
        for(int i = 0; i < MAX_CLIENTS; i++)
        {
                ...
                if(Predict() && (i == m_Snap.m_LocalClientId || IsDummyLocal || (AntiPingPlayers() && !IsOtherTeam(i))) && pChar)
                {
                        ...
                        if(i == m_Snap.m_LocalClientId || IsDummyLocal)
                        {
...
{
        if(!Client()->AnyDummyConnected())
                return;

        // Reset input for all connected non-active slots.
        // Original DDNet resets !g_Config.m_ClDummy (the non-active slot).
        // For multi-dummy, reset ALL non-active connected slots including slot 0
        // (when the active player is a dummy, slot 0 is non-active).
        for(int D = 0; D < MAX_DUMMIES; D++)
        {
                if(D == g_Config.m_ClDummy)
                        continue;
                ...
                if((m_aDummyInput[D].m_Fire & 1) != 0)
                        m_aDummyInput[D].m_Fire++;

                m_Controls.ResetInput(D);
                m_Controls.m_aInputData[D].m_Hook = 0;
                m_Controls.m_aInputData[D].m_Fire = m_aDummyInput[D].m_Fire;
