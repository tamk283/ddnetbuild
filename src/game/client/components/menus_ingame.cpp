...
void CMenus::RenderGame(CUIRect MainView)
{
        ...
        else if(Client()->AnyDummyConnectingDelayed())
        {
                DoButton_Menu(&s_DummyButton, Localize("Connect Dummy"), 1, &Button);
                GameClient()->m_Tooltips.DoToolTip(&s_DummyButton, &Button, Localize("Please wait…"));
        }
        else if(Client()->AnyDummyConnecting())
        {
                DoButton_Menu(&s_DummyButton, Localize("Connecting dummy"), 1, &Button);
        }
        else if(DoButton_Menu(&s_DummyButton, Client()->AnyDummyConnected() ? Localize("Disconnect All Dummies") : Localize("Connect Dummy 1"), 0, &Button))
        {
                if(!Client()->AnyDummyConnected())
                {
                        Client()->DummyConnect(1);
                }
                else
                {
                        if(GameClient()->CurrentRaceTime() / 60 >= g_Config.m_ClConfirmDisconnectTime && g_Config.m_ClConfirmDisconnectTime >= 0)
                        ...
                        else
                        {
                                ...
                                for(int D = 1; D < MAX_DUMMIES; D++)
                                {
                                        if(Client()->DummyConnected(D))
                                                Client()->DummyDisconnect(D, nullptr);
                                }
                                SetActive(false);
                        }
                }
        ...
        // Quick dummy switch buttons (D1-D7)
        if(Client()->DummyAllowed())
        {
                for(int D = 1; D < MAX_DUMMIES; D++)
                {
                        if(!Client()->DummyConnected(D))
                                continue;
                        ButtonBar.VSplitRight(3.0f, &ButtonBar, nullptr);
                        ButtonBar.VSplitRight(30.0f, &ButtonBar, &Button);
                        static CButtonContainer s_aDummySwitchButtons[MAX_DUMMIES];
                        char aLabel[32];
                        str_format(aLabel, sizeof(aLabel), "D%d", D);
                        int Active = (g_Config.m_ClDummy == D) ? 1 : 0;
                        if(DoButton_Menu(&s_aDummySwitchButtons[D], aLabel, Active, &Button))
                        {
                ...
                // Switch back to main button
                if(g_Config.m_ClDummy != 0)
                {
                        ButtonBar.VSplitRight(3.0f, &ButtonBar, nullptr);
                        ButtonBar.VSplitRight(50.0f, &ButtonBar, &Button);
                        static CButtonContainer s_MainSwitchButton;
                        if(DoButton_Menu(&s_MainSwitchButton, "Main", 0, &Button))
                        {
        ...
        if(GameClient()->m_Snap.m_pLocalInfo && GameClient()->m_Snap.m_pGameInfoObj && !Paused && !Spec)
        {
                if(GameClient()->m_Snap.m_pLocalInfo->m_Team != TEAM_SPECTATORS)
                {
                        ...
                        if(!Client()->AnyDummyConnecting() && DoButton_Menu(&s_SpectateButton, Localize("Spectate"), 0, &Button))
                        {
                                if(g_Config.m_ClDummy == 0 || Client()->AnyDummyConnected())
                                {
                ...
                if(GameClient()->IsTeamPlay())
                {
                        if(GameClient()->m_Snap.m_pLocalInfo->m_Team != TEAM_RED)
                        {
                                ...
                                if(!Client()->AnyDummyConnecting() && DoButton_Menu(&s_JoinRedButton, Localize("Join red"), 0, &Button))
                                {
                        ...
                        if(GameClient()->m_Snap.m_pLocalInfo->m_Team != TEAM_BLUE)
                        {
                                ...
                                if(!Client()->AnyDummyConnecting() && DoButton_Menu(&s_JoinBlueButton, Localize("Join blue"), 0, &Button))
                                {
                ...
                else
                {
                        if(GameClient()->m_Snap.m_pLocalInfo->m_Team != TEAM_GAME)
                        {
                                ...
                                if(!Client()->AnyDummyConnecting() && DoButton_Menu(&s_JoinGameButton, Localize("Join game"), 0, &Button))
                                {
