...
float CPlayers::GetPlayerTargetAngle(
        ...
        float Intra)
{
        bool IsDummyId = false;
        int MatchingDummy = -1;
        if(GameClient()->PredictDummy())
        {
        ...
        if(IsDummyId)
        {
                const CNetObj_PlayerInput &Input = g_Config.m_ClDummyHammer ? GameClient()->m_aHammerInput[MatchingDummy] : GameClient()->m_aDummyInput[MatchingDummy];
                return angle(vec2(Input.m_TargetX, Input.m_TargetY));
        }

        bool IsDummyCopyId = false;
        ...
        if((GameClient()->m_Snap.m_LocalClientId == ClientId || IsDummyCopyId) &&
                !GameClient()->m_Snap.m_SpecInfo.m_Active && Client()->State() != IClient::STATE_DEMOPLAYBACK)
        {
                // Fake Aim: if active and show_for_me, use fake offset for character
                if(GameClient()->m_Controls.m_FakeAimRenderActive &&
                   ((g_Config.m_KxFakeAimShowForMe && g_Config.m_KxFakeAim) ||
                    GameClient()->m_Tas.FakeAimActive()))
                {
        ...
        int ClientId)
{
        ...
        {
                bool IsDummyCopyClientId = false;
                if(GameClient()->PredictDummy() && g_Config.m_ClDummyCopyMoves)
                {
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                ...
                                if(GameClient()->m_aLocalIds[D] == ClientId)
                                {
                                        IsDummyCopyClientId = true;
                                        break;
                                }
                        }
                }
                if(IsDummyCopyClientId)
        ...
        {
                bool IsDummyCopyClientId2 = false;
                if(GameClient()->PredictDummy() && g_Config.m_ClDummyCopyMoves)
                {
                        for(int D = 0; D < MAX_DUMMIES; D++)
                        {
                                ...
                                if(GameClient()->m_aLocalIds[D] == ClientId)
                                {
                                        IsDummyCopyClientId2 = true;
                                        break;
                                }
                        }
                }
                if(IsDummyCopyClientId2 && GameClient()->m_Controls.m_aShowHookColl[g_Config.m_ClDummy] &&
        ...
        float Intra)
{
        ...
        if(ClientId == -1) // TAS fake tee: the attack age is fake-tick based, interpolated by the caller
        {
                AttackTime = mix((float)Prev.m_AttackTick, (float)Player.m_AttackTick, Intra) / (float)Client()->GameTickSpeed();
...

        {
                ...
                if(g_Config.m_ClAfkEmote && GameClient()->m_aClients[ClientId].m_Afk && !IsAnyDummyId)
                {
...
void CPlayers::OnRender()
{
        ...
        // TAS fake world: hide all real players, the fake tee renders instead.
        ...
        for(int i = 0; i < MAX_CLIENTS; ++i)
        {
                ...
                for(int D = 0; D < MAX_DUMMIES; D++)
                {
                        if((D == 0 || Client()->DummyConnected(D)) && i == GameClient()->m_aLocalIds[D])
                        {
                                IsLocalPlayer = true;
                                break;
                        }
        ...
        // v1.56.178: trajectory prediction moved to CTrajectory component
        // (GameClient()->m_Trajectory.OnRender) — called from gameclient render order.
}
