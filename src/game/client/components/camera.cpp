...
void CCamera::UpdateCamera()
{
        ...
        else
        {
                ...
                if(CanUseCameraInfo)
                {
                        ...
                        {
                                if(GameClient()->Client()->DummyConnected(D) && GameClient()->m_Snap.m_SpecInfo.m_SpectatorId == GameClient()->m_aLocalIds[D])
                                {
                                        CanUseCameraInfo = false;
                                        break;
                                }
                        }
                }
...
void CCamera::OnRender()
{
        ...
        // TAS fake world: camera follows the fake tee while joined.
        if(GameClient()->m_Tas.IsJoined() && !GameClient()->m_Tas.IsDead() &&
                !GameClient()->m_Snap.m_SpecInfo.m_Active)
        {
                m_Center = GameClient()->m_Tas.PlaygroundTeeRenderPos() + m_aDyncamCurrentCameraOffset[g_Config.m_ClDummy];
        }
...
bool CCamera::CanUseAutoSpecCamera() const
{
        ...
        bool IsLocalSpectator = (GameClient()->m_Snap.m_SpecInfo.m_SpectatorId == GameClient()->m_aLocalIds[0]);
        if(!IsLocalSpectator)
        {
                for(int D = 1; D < MAX_DUMMIES; D++)
                {
        ...
        return g_Config.m_ClSpecAutoSync && GameClient()->m_Snap.m_SpecInfo.m_HasCameraInfo && !IsLocalSpectator;
}
