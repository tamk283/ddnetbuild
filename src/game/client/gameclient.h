...
#include "components/kinetix/avoid_freeze.h"
...
#include "components/kinetix/irc.h"
#include "components/kinetix/trajectory.h" // v1.56.178: per-type trajectory prediction component
#include "components/kinetix/tas.h"
#include "components/broadcast.h"
#include "components/camera.h"
#include "components/censor.h"
#include "components/chat.h"
#include "components/kinetix/code_exec/code_exec.h"
...
public:
        ...
        CEsp m_Esp;
        CTrajectory m_Trajectory; // v1.56.178: per-type trajectory prediction (Tee/Pistol/Shotgun/Grenade/Laser)
...
private:
        ...
        int m_PredictedTick;
        int m_aLastNewPredictedTick[MAX_DUMMIES];
        ...
        int m_aCheckInfo[MAX_DUMMIES];

        mutable char m_aDDNetVersionStr[64];
...
public:
        ...
        CSnapState m_Snap;
        int m_aLocalTuneZone[MAX_DUMMIES]; // current tunezone (0-255)
        bool m_aReceivedTuning[MAX_DUMMIES]; // was tuning message received after zone change
        int m_aExpectingTuningForZone[MAX_DUMMIES]; // tunezone changed, waiting for tuning for that zone
        int m_aExpectingTuningSince[MAX_DUMMIES]; // how many snaps received since tunezone changed
        CTuningParams m_aTuning[MAX_DUMMIES]; // current local player tuning, only what the player/dummy has
        ...
        void OnDummyDisconnect(int Dummy = 1) override;
        ...
        // Apply copy-moves transformations (filter, mirror direction, mirror aim,
        // smart aim) to a non-active dummy's input. Called after m_aDummyInput[D]
        // is set from the player's input. Only active when cl_dummy_copy_moves is on.
        // v1.56.186: SrcInput parameter — the (possibly delayed) source input used
        // for aim transforms. When Copy Moves Latency > 0 this is the delayed entry,
        // so mirror/smart aim is replayed with the same delay as movement/fire.
        void ApplyCopyMovesTransforms(int D, const CNetObj_PlayerInput &SrcInput);

        // v1.56.186: Copy Moves Latency — buffer of recent active-player inputs.
        // When kx_copy_moves_latency > 0, inactive dummies replay the active player's
        // input from N ticks ago (N = latency_ms * dummy_index / tick_time_ms).
        // Recording happens once per SnapInput; lookup is per-dummy.
        // GetDelayedActiveInput returns the entry with tick >= (current - delayTicks)
        // closest to that target — i.e. the most recent entry older than delayTicks.
        // Returns false if no suitable entry exists (caller should fall back to live).
        struct SInputHistoryEntry
        {
                int m_Tick; // game tick when entry was recorded
                CNetObj_PlayerInput m_Input;
        };
        std::vector<SInputHistoryEntry> m_ActiveInputHistory;
        void RecordActiveInput(const CNetObj_PlayerInput &Input);
        bool GetDelayedActiveInput(int DelayTicks, CNetObj_PlayerInput &Out) const;
        ...
        // Rebuilds m_aDDNetVersionStr from current spoof config. Called lazily
        // from DDNetVersionStr() so the string is always up-to-date when sent.
        void UpdateDDNetVersionStr() const;
        ...
        void SendStartInfo7(int Dummy);
        void SendSkinChange7(int Dummy);
        // Returns true if the requested skin change got applied by the server
        bool GotWantedSkin7(int Dummy);
        void SendInfo(bool Start);
        void SendDummyInfo(bool Start, int Dummy = 1) override;
        void SendKill() const;
        void SendReadyChange7();

        void ApplyPreInputs(int Tick, bool Direct, CGameWorld &GameWorld);

        int m_aNextChangeInfo[MAX_DUMMIES];

        // DDRace

        int m_aLocalIds[MAX_DUMMIES];
        CNetObj_PlayerInput m_aDummyInput[MAX_DUMMIES]; // per-dummy stored input (index 0 unused for main)
        CNetObj_PlayerInput m_aHammerInput[MAX_DUMMIES]; // per-dummy hammerfly input
        unsigned int m_aDummyFire[MAX_DUMMIES]; // per-dummy fire counter
        int m_aDummyLastFireTick[MAX_DUMMIES] = {};
...
private:
        ...
        bool m_aDDRaceMsgSent[MAX_DUMMIES];
        int m_aShowOthers[MAX_DUMMIES];
        int m_aEnableSpectatorCount[MAX_DUMMIES]; // current setting as sent to the server, -1 if not yet sent
        ...
        int m_LastActiveDummy = 0; // Track which dummy was last active for swap fire transfer
        CCharOrder m_CharOrder;
        int m_aSwitchStateTeam[MAX_DUMMIES];
