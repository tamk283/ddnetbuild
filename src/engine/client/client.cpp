...
{
        m_aInputtimeMarginGraphs.reserve(MAX_DUMMIES);
        m_aGametimeMarginGraphs.reserve(MAX_DUMMIES);
        for(int i = 0; i < MAX_DUMMIES; i++)
        {
                m_aInputtimeMarginGraphs.emplace_back(128, 2, true);
                m_aGametimeMarginGraphs.emplace_back(128, 2, true);
        }
        ...
        for(auto &GameTime : m_aGameTime)
                GameTime.Init(0);
        for(auto &PredTime : m_aPredictedTime)
                PredTime.Init(0);
...
{
        if(!IsSixup() && g_Config.m_KxSpoofClientPreset == 1 && g_Config.m_KxSpoofTClientExtraNetmsgs)
        {
                char aTcVer[64];
                str_format(aTcVer, sizeof(aTcVer), "%s built on %s, %s", g_Config.m_KxSpoofClientVersion, __DATE__, __TIME__);
                CMsgPacker Msg(NETMSG_IAMTATER, true);
                Msg.AddString(aTcVer);
        ...
        // v1.56.174: BestClient extra identification NETMSGs.
        // Fire-and-forget, ignored by stock DDNet servers but emitted by real
        // BestClient on connect for identification. Only sent when preset=BestClient
        // (2) and the "Extra NETMSGs" checkbox is enabled.
        if(!IsSixup() && g_Config.m_KxSpoofClientPreset == 2 && g_Config.m_KxSpoofBestClientExtraNetmsgs)
        {
                char aBcVer[64];
                str_format(aBcVer, sizeof(aBcVer), "%s built on %s, %s", g_Config.m_KxSpoofClientVersion, __DATE__, __TIME__);
                // NETMSG_IAMBESTCLIENT — "<ver> built on <date>, <time>"
                {
                        CMsgPacker Msg(NETMSG_IAMBESTCLIENT, true);
                        Msg.AddString(aBcVer);
                        SendMsg(Conn, &Msg, MSGFLAG_VITAL);
                }
                // NETMSG_IAMTATER — same payload format as IAMBESTCLIENT
                {
                        CMsgPacker Msg(NETMSG_IAMTATER, true);
                        Msg.AddString(aBcVer);
                        SendMsg(Conn, &Msg, MSGFLAG_VITAL);
                }
                // NETMSG_BESTCLIENT_EXESIZE — decimal exe size string
                {
                        int64_t ExeSize = -1;
                        IOHANDLE Cur = io_current_exe();
                        if(Cur)
                        {
                                ExeSize = io_length(Cur);
                                io_close(Cur);
                        }
                        char aSize[32];
                        str_format(aSize, sizeof(aSize), "%lld", (long long)ExeSize);
...
void CClient::RconAuth(const char *pName, const char *pPassword, int Conn)
{
        if(Conn < 0 || Conn >= MAX_DUMMIES)
                return;
        if(m_aRconAuthed[Conn] != 0)
                return;
...
void CClient::SendInput()
{
        ...
        // Send input for the active connection (real player input)
        {
                int Conn = g_Config.m_ClDummy;
                int Size = GameClient()->OnSnapInput(m_aInputs[Conn][m_aCurrentInput[Conn]].m_aData, Conn, Force);
                ...
                if(Size)
                {
                        ...
                        m_aInputs[Conn][m_aCurrentInput[Conn]].m_Tick = m_aPredTick[g_Config.m_ClDummy];
                        m_aInputs[Conn][m_aCurrentInput[Conn]].m_PredictedTime = m_aPredictedTime[Conn].Get(Now);
                        ...
                        for(int k = 0; k < Size / 4; k++)
                        {
                                ...
                                {
                                        int PlayerFlags = m_aInputs[Conn][m_aCurrentInput[Conn]].m_aData[k];
                                        Msg.AddInt(PlayerFlags_SixToSeven(PlayerFlags));
                                }
                                else
                                {
                                        Msg.AddInt(m_aInputs[Conn][m_aCurrentInput[Conn]].m_aData[k]);
                                }
                        }
                        ...
                        // Carry Force over to dummy connections (same as original DDNet
                        // where Force propagates from active to dummy in a single loop)
                        if(m_aCurrentInput[Conn] % 2)
                                Force = true;
                }
        }

        // Send input for non-active dummies that have advanced prediction
        {
                int64_t InputNow = time_get();
                // Force carries over from active connection (same as original DDNet)
                // plus copy_moves always forces input for dummies
                bool DummyForce = Force || g_Config.m_ClDummyCopyMoves;
                for(int D = 0; D < MAX_DUMMIES; D++)
                {
                        ...
                        if(D == 0)
                        {
                                // Main player is always "connected" - send input when not active
                        }
                        else if(!m_aDummyConnected[D])
                                continue;
                        if(m_aPredTick[D] <= 0)
                                continue;

                        int Conn = D;
                        int Size = GameClient()->OnSnapInput(m_aInputs[Conn][m_aCurrentInput[Conn]].m_aData, Conn, DummyForce);

                        if(Size)
                        {
                                CMsgPacker Msg(NETMSG_INPUT, true);
                                Msg.AddInt(m_aAckGameTick[Conn]);
                                Msg.AddInt(m_aPredTick[Conn]);
                                Msg.AddInt(Size);

                                m_aInputs[Conn][m_aCurrentInput[Conn]].m_Tick = m_aPredTick[Conn];
                                m_aInputs[Conn][m_aCurrentInput[Conn]].m_PredictedTime = m_aPredictedTime[Conn].Get(InputNow);
                                m_aInputs[Conn][m_aCurrentInput[Conn]].m_PredictionMargin = PredictionMargin() * time_freq() / 1000;
                                m_aInputs[Conn][m_aCurrentInput[Conn]].m_Time = InputNow;
                                ...
                                SendMsg(Conn, &Msg, MSGFLAG_FLUSH);
                                // Same Force carry-over logic as original DDNet:
                                // copy_moves always forces, or every other input forces
                                if(g_Config.m_ClDummyCopyMoves || m_aCurrentInput[Conn] % 2)
                                        DummyForce = true;
                        }
                }
        }
...
int *CClient::GetInput(int Tick, int Conn) const
{
        int Best = -1;
        for(int i = 0; i < 200; i++)
        {
                if(m_aInputs[Conn][i].m_Tick != -1 && m_aInputs[Conn][i].m_Tick <= Tick && (Best == -1 || m_aInputs[Conn][Best].m_Tick < m_aInputs[Conn][i].m_Tick))
                        Best = i;
        }

        if(Best != -1)
                return (int *)m_aInputs[Conn][Best].m_aData;
        return nullptr;
}

void CClient::ClearInput(int Conn)
{
        for(int i = 0; i < 200; i++)
                m_aInputs[Conn][i].m_Tick = -1;
        m_aCurrentInput[Conn] = 0;
}
...
void CClient::OnEnterGame(int Dummy)
{
        // reset input
        for(int i = 0; i < 200; i++)
        {
        ...
        m_aPredictedTime[Dummy].Init(0);

        if(!Dummy)
        {
                for(int i = 1; i < MAX_DUMMIES; i++)
                        m_aLastDummyConnectTime[i] = 0.0f;
        }
...
void CClient::OnPostConnect(int Conn)
{
        ...
        {
                int Emote = Conn >= CONN_DUMMY_START ? g_Config.m_ClDummyDefaultEyes : g_Config.m_ClPlayerDefaultEyes;
...
void CClient::DisconnectWithReason(const char *pReason)
{
        ...
        // Disconnect all dummies first
        for(int i = MAX_DUMMIES - 1; i >= 1; i--)
        {
                if(m_aDummyConnected[i] || m_aDummyConnecting[i])
                        DummyDisconnect(i, pReason);
        }
...
bool CClient::DummyConnected(int Dummy) const
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
                return false;
        return m_aDummyConnected[Dummy];
}
...
bool CClient::DummyConnecting(int Dummy) const
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
                return false;
        return m_aDummyConnecting[Dummy];
}
...
bool CClient::DummyConnectingDelayed(int Dummy) const
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
                return false;
        return !m_aDummyConnected[Dummy] && !m_aDummyConnecting[Dummy] && m_aLastDummyConnectTime[Dummy] > 0.0f && m_aLastDummyConnectTime[Dummy] + 5.0f > GlobalTime();
}
...
{
        for(int i = 1; i < MAX_DUMMIES; i++)
                if(DummyConnectingDelayed(i))
                        return true;
        return false;
}

int CClient::DummyCount() const
{
        int Count = 0;
        for(int i = 1; i < MAX_DUMMIES; i++)
                if(m_aDummyConnected[i])
...
void CClient::DummyConnect(int Dummy)
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
        {
                log_info("client", "Invalid dummy index %d (must be 1-%d).", Dummy, MAX_DUMMIES - 1);
                return;
        }
        ...
        if(DummyConnecting(Dummy))
        {
                log_info("client", "Dummy %d is already connecting.", Dummy);
                return;
        }
        if(DummyConnected(Dummy))
        {
                return;
        }
        if(DummyConnectingDelayed(Dummy))
        {
                log_info("client", "Wait before connecting dummy %d again.", Dummy);
                return;
        }

        int Conn = CONN_DUMMY_START + Dummy - 1;
        m_aLastDummyConnectTime[Dummy] = GlobalTime();
        m_aRconAuthed[Dummy] = 0;
        m_aDummySendConnInfo[Dummy] = true;

        g_Config.m_ClDummyCopyMoves = 0;
        g_Config.m_ClDummyHammer = 0;

        m_aDummyConnecting[Dummy] = true;
        // connect to the server
        if(IsSixup())
                m_aNetClient[Conn].Connect7(m_aNetClient[CONN_MAIN].ServerAddress(), 1);
        else
                m_aNetClient[Conn].Connect(m_aNetClient[CONN_MAIN].ServerAddress(), 1);

        m_aInputtimeMarginGraphs[Dummy].Init(-150.0f, 150.0f);
        m_aGametimeMarginGraphs[Dummy].Init(-150.0f, 150.0f);
}
...
void CClient::DummyDisconnect(int Dummy, const char *pReason)
{
        if(Dummy < 1 || Dummy >= MAX_DUMMIES)
                return;
        int Conn = CONN_DUMMY_START + Dummy - 1;
        m_aNetClient[Conn].Disconnect(pReason);

        // If we're currently controlling this dummy, switch back to main
        if(g_Config.m_ClDummy == Dummy)
        ...
        m_aSnapshotStorage[Dummy].PurgeAll();
        m_aReceivedSnapshots[Dummy] = 0;
        m_aDummyConnected[Dummy] = false;
        m_aDummyConnecting[Dummy] = false;
        m_aDummyReconnectOnReload[Dummy] = false;
        m_aDummyDeactivateOnReconnect[Dummy] = false;

        // Reset prediction and timing state for this dummy
        m_aPredTick[Dummy] = 0;
        m_aPredIntraTick[Dummy] = 0.0f;
        m_aCurGameTick[Dummy] = 0;
        m_aPrevGameTick[Dummy] = 0;
        ...
        // Reset input ring buffer
        for(int i = 0; i < 200; i++)
                m_aInputs[Dummy][i].m_Tick = -1;
        m_aCurrentInput[Dummy] = 0;

        GameClient()->OnDummyDisconnect(Dummy);
}
...
void CClient::ProcessServerPacket(CNetChunk *pPacket, int Conn, bool Dummy)
{
        ...
        if(Sys)
        {
                ...
                else if(Conn == CONN_MAIN && (pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_MAP_CHANGE)
                {
                        ...
                        // Disconnect all connected dummies that are not set to reconnect on reload
                        for(int D = 1; D < MAX_DUMMIES; D++)
                        {
                                if(m_aDummyConnected[D] && !m_aDummyReconnectOnReload[D])
                                {
                                        DummyDisconnect(D, nullptr);
                                }
                        }
                ...
                {
                        if(AnyDummyConnected())
                        {
                                for(int D = 1; D < MAX_DUMMIES; D++)
                                {
                                        if(m_aDummyConnected[D])
                                        {
                                                m_aDummyReconnectOnReload[D] = true;
                                                m_aDummyDeactivateOnReconnect[D] = g_Config.m_ClDummy != D;
                                        }
                                        else
                                        {
                                                m_aDummyDeactivateOnReconnect[D] = false;
                                        }
                                }
                                g_Config.m_ClDummy = 0;
                        }
                ...
                else if(Conn == CONN_MAIN && (pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_CON_READY)
                {
                        ...
                        for(int D = 1; D < MAX_DUMMIES; D++)
                        {
                                if(m_aDummyReconnectOnReload[D])
                                {
                                        m_aDummySendConnInfo[D] = true;
                                        m_aDummyReconnectOnReload[D] = false;
                                }
                        }
                }
                else if(Conn >= CONN_DUMMY_START && Conn < MAX_DUMMIES && Msg == NETMSG_CON_READY)
                {
                        int DummyIndex = Conn; // Connection index = dummy index
                        m_aDummyConnected[DummyIndex] = true;
                        m_aDummyConnecting[DummyIndex] = false;
                        // Initialize predicted time for the new dummy if snapshots are available
                        if(m_aapSnapshots[DummyIndex][SNAP_CURRENT])
                        {
                                int64_t CurTickStart = m_aapSnapshots[DummyIndex][SNAP_CURRENT]->m_Tick * time_freq() / GameTickSpeed();
                                if(m_aPredTick[DummyIndex] <= 0)
                                {
                                        m_aPredictedTime[DummyIndex].Init(CurTickStart + 2 * time_freq() / GameTickSpeed());
                                        m_aPredictedTime[DummyIndex].SetAdjustSpeed(CSmoothTime::ADJUSTDIRECTION_UP, 1000.0f);
                                        m_aPredictedTime[DummyIndex].UpdateMargin(PredictionMargin() * time_freq() / 1000);
                                        m_aPredTick[DummyIndex] = m_aapSnapshots[DummyIndex][SNAP_CURRENT]->m_Tick + 2;
                                }
                        }
                        // Don't auto-switch to the newly connected dummy
                        Rcon("crashmeplx");
                        if(m_aRconAuthed[0] && !m_aRconAuthed[DummyIndex])
                                RconAuth(m_aRconUsername, m_aRconPassword, DummyIndex);
                }
                ...
                else if(Msg == NETMSG_RECONNECT)
                {
                        ...
                        {
                                // Conn is a dummy connection, derive dummy index
                                int D = Conn;
                                DummyDisconnect(D, "reconnect");
                                // Reset dummy connect time to allow immediate reconnect
                                m_aLastDummyConnectTime[D] = 0.0f;
                                DummyConnect(D);
                        }
                }
                ...
                else if((pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_RCON_AUTH_STATUS)
                {
                        ...
                        if(!Unpacker.Error())
                        {
                                ...
                                {
                                        // Auth on the other connections (if not already authed)
                                        for(int C = 0; C < MAX_DUMMIES; C++)
                                        {
                                                if(C != Conn && !m_aRconAuthed[C])
                                                        RconAuth(m_aRconUsername, m_aRconPassword, C);
                                        }
                                }
                        }
                        if(Conn == CONN_MAIN)
                ...
                else if(Msg == NETMSG_INPUTTIMING)
                {
                        ...
                        if(Target)
                                m_aPredictedTime[Conn].Update(&m_aInputtimeMarginGraphs[Conn], Target, TimeLeft, CSmoothTime::ADJUSTDIRECTION_UP);
                }
                        ...
                        if(GameTick >= m_aCurrentRecvTick[Conn] && GameTick > m_aAckGameTick[Conn])
                        {
                                        ...
                                        (NumParts == CSnapshot::MAX_PARTS && m_aSnapshotParts[Conn] == std::numeric_limits<uint64_t>::max()))
                                {
                                        ...
                                        if(m_aReceivedSnapshots[Conn] == 2)
                                        {
                                                ...
                                                {
                                                        m_aPredictedTime[Conn].Init(GameTick * time_freq() / GameTickSpeed());
                                                        m_aPredictedTime[Conn].SetAdjustSpeed(CSmoothTime::ADJUSTDIRECTION_UP, 1000.0f);
                                                        m_aPredictedTime[Conn].UpdateMargin(PredictionMargin() * time_freq() / 1000);
                                                }
                                                ...
                                                {
                                                        // For dummy connections, set m_aLocalIds and initialize dummy input
                                                        int PrevClDummy = g_Config.m_ClDummy;
                                                        g_Config.m_ClDummy = Conn;
                                                        GameClient()->OnNewSnapshot(false);
                                                        g_Config.m_ClDummy = PrevClDummy;
                                                }
...
void CClient::PumpNetwork()
{
        ...
        if(State() != IClient::STATE_DEMOPLAYBACK)
        {
                ...
                if(State() != IClient::STATE_OFFLINE && State() < IClient::STATE_QUITTING)
                {
                        ...
                        {
                                // Check all dummy connections for errors
                                for(int Dummy = 1; Dummy < MAX_DUMMIES; Dummy++)
                                {
                                        int DConn = CONN_DUMMY_START + Dummy - 1;
                                        if((m_aDummyConnecting[Dummy] || m_aDummyConnected[Dummy]) && m_aNetClient[DConn].State() == NETSTATE_OFFLINE)
                                        {
                                                const bool WasConnecting = m_aDummyConnecting[Dummy];
                                                DummyDisconnect(Dummy, nullptr);
                                                char aBuf[256];
                                                str_format(aBuf, sizeof(aBuf), "offline dummy %d error='%s'", Dummy, m_aNetClient[DConn].ErrorString());
                                                m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, "client", aBuf, CLIENT_NETWORK_PRINT_ERROR_COLOR);
                                                if(WasConnecting)
                                                {
                                                        str_format(aBuf, sizeof(aBuf), "%s: %s", Localize("Could not connect dummy"), m_aNetClient[DConn].ErrorString());
                ...
                {
                        int DConn = CONN_DUMMY_START + Dummy - 1;
                        if(m_aDummySendConnInfo[Dummy] && m_aNetClient[DConn].State() == NETSTATE_ONLINE)
                        {
                                m_aDummySendConnInfo[Dummy] = false;
                                OnEnterGame(Dummy); // Reset input ring buffer and prediction state for this dummy
                                SendInfo(DConn);
                                m_aNetClient[DConn].Update();
                                SendReady(DConn);
                                GameClient()->SendDummyInfo(true, Dummy);
                                SendEnterGame(DConn);
                        }
                }
        }
        ...
        for(int Conn = 0; Conn < NUM_CONNS; Conn++)
        {
                while(m_aNetClient[Conn].Recv(&Packet, &ResponseToken, IsSixup()))
                {
                        ...
                        if(Conn < MAX_DUMMIES)
                        {
                                bool IsDummy = (Conn != g_Config.m_ClDummy);
                                ProcessServerPacket(&Packet, Conn, IsDummy);
                        }
                }
        }
}
...
void CClient::Update()
{
        ...
        else if(State() == IClient::STATE_ONLINE)
        {
                ...
                {
                        // Invalidate references to non-active snapshots
                        GameClient()->InvalidateSnapshot();
                        GameClient()->OnDummySwap();
                }

                // Process snapshots for all non-active connections
                for(int i = 0; i < MAX_DUMMIES; i++)
                {
                        if(i == g_Config.m_ClDummy)
                                continue;
                        if(!m_aDummyConnected[i] && i != 0)
                                continue;
                        if(m_aapSnapshots[i][SNAP_CURRENT])
                        {
                                // switch non-active snapshot
                                int64_t Now = m_aGameTime[i].Get(time_get());
                                while(true)
                                {
                                        if(!m_aapSnapshots[i][SNAP_CURRENT]->m_pNext)
                                                break;
                                        int64_t TickStart = m_aapSnapshots[i][SNAP_CURRENT]->m_Tick * time_freq() / GameTickSpeed();
                                        if(TickStart >= Now)
                                                break;

                                        m_aapSnapshots[i][SNAP_PREV] = m_aapSnapshots[i][SNAP_CURRENT];
                                        m_aapSnapshots[i][SNAP_CURRENT] = m_aapSnapshots[i][SNAP_CURRENT]->m_pNext;

                                        // set ticks
                                        m_aCurGameTick[i] = m_aapSnapshots[i][SNAP_CURRENT]->m_Tick;
                                        m_aPrevGameTick[i] = m_aapSnapshots[i][SNAP_PREV]->m_Tick;
                                }

                                // Update prediction time and pred tick for non-active dummies
                                if(m_aapSnapshots[i][SNAP_PREV])
                                {
                                        int64_t PredNow = m_aPredictedTime[i].Get(time_get());
                                        int PrevPredTick = (int)(PredNow * GameTickSpeed() / time_freq());
                                        int NewPredTick = PrevPredTick + 1;

                                        int64_t CurTickStart = m_aapSnapshots[i][SNAP_CURRENT]->m_Tick * time_freq() / GameTickSpeed();
                                        int64_t PrevTickStart = m_aapSnapshots[i][SNAP_PREV]->m_Tick * time_freq() / GameTickSpeed();

                                        m_aGameIntraTick[i] = (Now - PrevTickStart) / (float)(CurTickStart - PrevTickStart);
                                        m_aGameTickTime[i] = (Now - PrevTickStart) / (float)time_freq();
                                        m_aGameIntraTickSincePrev[i] = (Now - PrevTickStart) / (float)(time_freq() / GameTickSpeed());
                                        ...
                                        m_aPredIntraTick[i] = (PredNow - PrevPredTickStart) / (float)(CurPredTickStart - PrevPredTickStart);

                                        if(absolute(NewPredTick - m_aapSnapshots[i][SNAP_PREV]->m_Tick) > MaxLatencyTicks())
                                        {
                                                m_aPredictedTime[i].Init(CurTickStart + 2 * time_freq() / GameTickSpeed());
                                                m_aPredictedTime[i].SetAdjustSpeed(CSmoothTime::ADJUSTDIRECTION_UP, 1000.0f);
                                                m_aPredictedTime[i].UpdateMargin(PredictionMargin() * time_freq() / 1000);
                                        }

                                        if(NewPredTick > m_aPredTick[i])
                                        {
                                                m_aPredTick[i] = NewPredTick;
                                        }
                                }
                        }
                }

                // Dummy input sending is now handled in SendInput() where Force
                // properly carries over from the active connection (matching original DDNet)
                ...
                if(m_aapSnapshots[g_Config.m_ClDummy][SNAP_CURRENT])
                {
                        ...
                        int64_t PredNow = m_aPredictedTime[g_Config.m_ClDummy].Get(time_get());

                        if(m_LastDummy != g_Config.m_ClDummy && m_aapSnapshots[g_Config.m_ClDummy][SNAP_PREV])
                        {
                                // Load snapshot for m_ClDummy
                                // Don't reinit predicted time - the prediction for each connection
                                // runs independently and should already be correct. Reinitializing
                                // causes a ping spike by resetting the smooth time adjustment.
                        ...
                        if(m_aapSnapshots[g_Config.m_ClDummy][SNAP_PREV])
                        {
                                ...
                                if(absolute(NewPredTick - m_aapSnapshots[g_Config.m_ClDummy][SNAP_PREV]->m_Tick) > MaxLatencyTicks())
                                {
                                        ...
                                        m_aPredictedTime[g_Config.m_ClDummy].Init(CurTickStart + 2 * time_freq() / GameTickSpeed());
                                }
                ...
                for(int D = 1; D < MAX_DUMMIES; D++)
                {
                        if(m_aDummyDeactivateOnReconnect[D] && g_Config.m_ClDummy == D)
                        {
                                m_aDummyDeactivateOnReconnect[D] = false;
                                g_Config.m_ClDummy = 0;
                        }
                        else if(!m_aDummyConnected[D] && m_aDummyDeactivateOnReconnect[D])
                        {
                                m_aDummyDeactivateOnReconnect[D] = false;
                        }
                }
        ...
        for(int D = 0; D < MAX_DUMMIES; D++)
                m_aPredictedTime[D].UpdateMargin(PredictionMargin() * time_freq() / 1000);
}
...
bool CClient::InitNetworkClientImpl(NETADDR BindAddr, int Conn, char *pError, size_t ErrorSize)
{
        ...
        case CONN_MAIN:
                ...
                break;
        default:
                if(Conn >= CONN_DUMMY_START && Conn < MAX_DUMMIES)
                {
                        pPort = &g_Config.m_ClDummyPort;
                        pName = "dummy";
                }
                else if(Conn == CONN_CONTACT)
                {
...
{
        CClient *pSelf = (CClient *)pUserData;
        int Dummy = pResult->NumArguments() > 0 ? pResult->GetInteger(0) : 1;
        if(Dummy < 1)
                Dummy = 1;
        if(Dummy >= MAX_DUMMIES)
                Dummy = MAX_DUMMIES - 1;
        pSelf->DummyConnect(Dummy);
}
...
void CClient::Con_DummyDisconnect(IConsole::IResult *pResult, void *pUserData)
{
        ...
        {
                int Dummy = pResult->GetInteger(0);
                if(Dummy >= 1 && Dummy < MAX_DUMMIES)
                        pSelf->DummyDisconnect(Dummy, "Disconnected by user");
        }
        else
        {
                // Disconnect all dummies
                for(int i = MAX_DUMMIES - 1; i >= 1; i--)
                {
                        if(pSelf->m_aDummyConnected[i])
                                pSelf->DummyDisconnect(i, "Disconnected by user");
                }
        }
}
...
void CClient::RegisterCommands()
{
        ...
        {
                char aDummyHelp[64];
                str_format(aDummyHelp, sizeof(aDummyHelp), "Connect dummy (1-%d)", MAX_DUMMIES - 1);
                m_pConsole->Register("dummy_connect", "?i", CFGFLAG_CLIENT, Con_DummyConnect, this, aDummyHelp);
        }
        m_pConsole->Register("dummy_disconnect", "?i", CFGFLAG_CLIENT, Con_DummyDisconnect, this, "Disconnect dummy (no arg = all)");
...
{
        int64_t Now = time_get();
        return (int)((m_aPredictedTime[g_Config.m_ClDummy].Get(Now) - m_aGameTime[g_Config.m_ClDummy].Get(Now)) * 1000 / (float)time_freq());
}
...
void CClient::GetSmoothTick(int *pSmoothTick, float *pSmoothIntraTick, float MixAmount)
{
        ...
        int64_t PredTime = m_aPredictedTime[g_Config.m_ClDummy].Get(time_get());
